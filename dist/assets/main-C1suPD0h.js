...
class MainMenu {
  ...
  bindClickOutside = () => {
    window.addEventListener("click", (e) => {
      if (!this.menu.contains(e.target) && e.target !== this.closeButton && this.isMenuOpened && !this.isLocked) {
        this.close();
      }
    });
  };
...
class HeaderSearch {
  ...
  bindClickOutside = () => {
    window.addEventListener("click", (e) => {
      if (!this.isLocked && this.isOpened && !this.searchWrapper.contains(e.target) && e.target !== this.openButton) {
        this.close();
      }
    });
  };
  ...
  static checkBreakpointChange() {
    ...
    if (this.currentBreakpoint !== this.previousBreakpoint) {
      this.listeners.forEach(
        (callback) => callback(this.previousBreakpoint, this.currentBreakpoint)
      );
      this.previousBreakpoint = this.currentBreakpoint;
    }
  }
...
class Global {
  ...
  devUtils = () => {
    ...
    box.innerHTML = Utils$1.currentBreakpoint;
    document.body.appendChild(box);
    Utils$1.onBreakpointChange(() => {
      box.innerHTML = Utils$1.currentBreakpoint;
    });
  };
}
class Home {
  ...
  init = () => {
    document.addEventListener("DOMContentLoaded", () => {
      ...
      this.bindProductsByCategorySection();
  ...
  bindFeaturedProductsSlider = () => {
    this.featuredProductsSlider = new Swiper(
  ...
  bindPartnersSlider = () => {
    this.partnersSlider = new Swiper(
